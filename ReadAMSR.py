...
def UnzipAMSR(AMSRzipfile, outfilepath):
    ...
    AMSRfile = open((outfilepath + AMSRzipfileshortname), "wb")
...
def AMSRtoCSV_Br89(AMSRzipfile, outfilepath, channelA, channelB, polarization):
    ...
    if(not os.path.exists(AMSRcsv) and  not os.path.isfile(AMSRcsv)):
...
def CSVtoRaster(AMSRzipfile, outfilepath, frequency, polarization):
    ...
    os.system('gdal_grid -a_srs EPSG:3411 -a average:radius1=' +
              str(radius1) + ':radius2=' + str(radius2) + ':min_points=1 -txe -3850000 3750000 -tye -5350000 5850000 -outsize '+
              str(xsize) + ' ' + str(ysize) + ' -l ' + AMSRcsv_shortname + ' '  + AMSRcsv_vrt + ' ' + AMSR_tif)
...
##############################################################################
outfilepath = "/home/hope/ipython/AMSR/"#'G:\\AMSR\\'
inputfilepath = "/home/hope/ipython/AMSR2/L1R/"#'U:\\AMSR2\\L1R\\'
startyear = 2013
endyear = 2013
startday = 1
startmonth = 1
...
while workingdate != (enddate + diff):
    ...
    folder = inputfilepath + file_searchstring
    filelist.extend(glob.glob(folder))
    workingdate = workingdate + diff
#########################################
## Calculate 89GHz
#########################################
